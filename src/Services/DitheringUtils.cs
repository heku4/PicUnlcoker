...
namespace PicUnlocker.Services
{
    internal static class DitheringUtils
    {
        ...
        public static void PerPixelQuantitation(string filePath, int paletteBitSize)
        {
            ...
            for (var y = 0; y < pixelsImage.Height; y++)
            for (var x = 0; x < pixelsImage.Width; x++)
            {
                ...
                var newPixelData = Color.FromArgb((int)r, (int)g, (int)b);
        ...
        public static void FloydSteinbergDithering(string filePath, int paletteBitSize)
        {
            ...
            var FirstErrorWeight = 7.0 / 16.0;
            var SecondErrorWeight = 3.0 / 16.0;
            var ThirdErrorWeight = 5.0 / 16.0;
            var LastErrorWeight = 1.0 / 16.0;
            ...
            for (var y = 0; y < pixelsImage.Height - 1; y++)
            for (var x = 1; x < pixelsImage.Width - 1; x++)
            {
        ...
        private static (int R, int G, int B) CalculateError(int channelRValue, int channelGValue, int channelBValue,
            int paletteBitSize)
        {
